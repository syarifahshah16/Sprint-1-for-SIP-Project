...
def createPcapEvidenceTable(conn):
    try:
        ...
        cursor.execute('''CREATE TABLE pcapEvidenceTable(id INTEGER PRIMARY KEY, frameNum INTEGER, filePath TEXT, srcHost TEXT, srcPort TEXT, dstHost TEXT, dstPort TEXT, protocol TEXT, filename TEXT, ext TEXT, size TEXT)''')
        conn.commit()
...
"""def insertPcapEvidenceDetails(conn, frameNum, filePath, srcHost, srcPort, dstHost, dstPort, protocol, filename, ext, size, timestamp):
    ...
    conn.commit()""" # not used due to effect of overheads on speed



def selectPcapEvidenceDetails(conn, id):
    cursor = conn.cursor()
    cursor.execute('''SELECT frameNum, filePath, srcHost, srcPort, dstHost, dstPort, protocol, filename, ext, size FROM pcapEvidenceTable WHERE id=?''', (id,)) # note trailing comma as we're passing a tuple with a single value
    row = cursor.fetchone()
    return row
